...
final class DefaultCamera: NSObject, Camera {
  ...
  private static func createConnection(
    ...
    // Apply video stabilization from user settings
    if connection.isVideoStabilizationSupported {
      let stabilizationMode = DefaultCamera.getPreferredStabilizationMode()
  ...
  /// Get the preferred stabilization mode from UserDefaults
  private static func getPreferredStabilizationMode() -> AVCaptureVideoStabilizationMode {
    if let rawValue = UserDefaults.standard.object(forKey: "retrocam_stabilization_mode") as? Int,
       let mode = AVCaptureVideoStabilizationMode(rawValue: rawValue) {
      return mode
    }
    return .off
  }
